...
if __name__ == "__main__":
    ...
    output_path = path_spine_mask[0][:-7] + '_DICOM_Orientation_final_3.nii.gz'
    ...
    # #Load Masks
    path_lesion_mask= [os.path.join(patient_main_file, f) for f in os.listdir(patient_main_file) if f.endswith('validation_VV_final.nii.gz')]    
    
    dicom_folder = join(patient_main_file,'S207300')
    nifti_path = path_lesion_mask[0]
    output_path = path_lesion_mask[0][:-7] + '_DICOM_Orientation_final_3.nii.gz'
    reorient_to_DICOM_orientation(dicom_folder,nifti_path,output_path)
    '''
    # Načtení objemu a afinní matice
    image, affine_matrix = load_dicom_series(dicom_folder)
