...
def find_trabecular_spine_mask(base_segmentation, subject):
    """Vrátí cestu ke správné NIfTI masce: rev_Myel soubor pokud existuje, jinak nnUNet _cor soubor."""
    masks = subfiles(join(base_segmentation, subject), join=True, prefix="rev_Myel", suffix=".nii.gz")
    if masks:
        return masks[0]

    nnunet_file = f"{subject}_trabecular_spine_segmentation_nnUNet_v_1_0_cor.nii.gz"
    nnunet_path = join(base_segmentation, subject, nnunet_file)
    if os.path.exists(nnunet_path):
        return nnunet_path

    raise FileNotFoundError(
        f"Nenašel jsem masku pro {subject}. Očekával jsem buď soubor s prefixem 'rev_Myel' nebo {nnunet_file} v {join(base_segmentation, subject)}"
    )
